...
import { runSeededSqliteHttpPaymentDemo } from "./sqlite-demo";

test("SQLite AFAL HTTP payment demo returns a compact summary for the canonical payment request", async () => {
  const dir = await mkdtemp(join(tmpdir(), "afal-sqlite-http-demo-"));

  try {
    const result = await runSeededSqliteHttpPaymentDemo(dir);
