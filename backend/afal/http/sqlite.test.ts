...
import { createSeededSqliteAfalBundle } from "../service";
import { createSeededSqliteAfalHttpRouter } from "./sqlite";
import { AFAL_HTTP_ROUTES } from "./types";

test("SQLite AFAL HTTP router executes payment flow and persists state to integration stores", async () => {
  const dir = await mkdtemp(join(tmpdir(), "afal-sqlite-http-"));

  try {
    const first = createSeededSqliteAfalHttpRouter(dir);
    ...
    const second = createSeededSqliteAfalHttpRouter(dir);
...
test("SQLite AFAL HTTP router applies and resumes approval sessions across restarts", async () => {
  const dir = await mkdtemp(join(tmpdir(), "afal-sqlite-http-approval-"));

  try {
    const bundle = createSeededSqliteAfalBundle(dir);
    ...
    const applyResponse = await first.router.handle({
      ...
      body: {
        requestRef: "req-sqlite-approval-apply-001",
        input: {
    ...
    const resumeResponse = await second.router.handle({
      ...
      body: {
        requestRef: "req-sqlite-approval-resume-001",
        input: {
          approvalSessionRef: approvalRequest.approvalSession.approvalSessionId,
        },
      },
    });
    const amnStore = new SqliteAmnStore({ filePath: second.paths.amn });
