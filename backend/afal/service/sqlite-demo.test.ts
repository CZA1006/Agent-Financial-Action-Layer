...
import { runSeededSqliteAfalDemo } from "./sqlite-demo";

test("SQLite AFAL demo returns a compact summary for the seeded integration mode", async () => {
  const dir = await mkdtemp(join(tmpdir(), "afal-sqlite-demo-"));

  try {
    const result = await runSeededSqliteAfalDemo(dir);
